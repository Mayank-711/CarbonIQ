...
const Calculator = () => {
  ...
  const fetchSuggestions = async (input, setSuggestions, setCoordinate) => {
  ...
  const fetchRouteDetails = async () => {
    if (!sourceCoordinates || !destinationCoordinates) {
      console.error("Coordinates for source and destination are required.");
      return;
    }
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const totalDistance = legs.reduce((sum, leg) => sum + leg.distance, 0) / 1000; // Convert to km
        const totalDuration = legs.reduce((sum, leg) => sum + leg.duration, 0) / 60; // Convert to minutes
        ...
        console.log("Time Taken:", totalDuration.toFixed(2) + " mins");
        console.log('Date:',currentDate);

        // Call the API with route details
        ...
        const apiResponse = await axios.post("http://localhost:5000/api/predict_co2", predictionData);
        console.log("API Prediction Response:", apiResponse.data);
      } else {
        ...
        <label className="block mb-4">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              fetchSuggestions(e.target.value, setSourceSuggestions, setSourceCoordinates);
            }}
          />
          <ul className="bg-white text-green-800 mt-1 rounded-md shadow-lg">
        ...
        <label className="block mb-4">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              fetchSuggestions(e.target.value, setDestinationSuggestions, setDestinationCoordinates);
            }}
          />
          <ul className="bg-white text-green-800 mt-1 rounded-md shadow-lg">
